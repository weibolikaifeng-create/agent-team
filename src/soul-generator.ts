 ...
 */
export function generateAgentsMd(params: SoulMdParams): string {
...
注意事项：首次更新进展时你可以先从\`__execDir__/todo.md\`读取进展条目后再更新步骤，以免更新出错。进展的更新必须使用\`team_update_progress\`工具完成，不允许直接编辑todo文件，工具会返回todo文件更新后的状态，如果发现更新错误请及时纠正。
 ...
 */
export function generateAgentsMdWeb(params: SoulMdParams): string {
  const workerList = params.workers
    .map((w) => `- **${w.role}** (\`${w.id}\`): ${w.responsibility}`)
    .join("\n");
