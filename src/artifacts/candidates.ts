...
function hasDeliverableExtension(value: string): boolean {
...
export function extractPathCandidates(text: string): string[] {
  ...
  for (const pattern of PATH_PATTERNS) {
    for (const match of text.matchAll(pattern)) {
      ...
      if (isLikelyPathToken(raw)) out.add(raw);
    }
  }
 ...
 */
export async function enrichIfArtifact(
  absPath: string,
  opts: { startedAtMs: number },
): Promise<EnrichedArtifact | null> {
  if (isIgnoredPath(absPath)) return null;
  ...
  if (Number.isFinite(opts.startedAtMs) && stat.mtimeMs < opts.startedAtMs) return null;
