...
import {
  resolveMemberBySessionKey,
  describeOutputOwner,
  type MemberIdentity,
  type MemberRole,
} from "./members-registry.js";
...
/** Owner (execution + member) an artifact is attributed to. */
type Owner = {
  executionId: string;
  memberId: string;
  role: MemberRole;
 ...
 */
function ownerFromPath(stateDir: string, teamId: string, absPath: string): Owner | undefined {
  ...
  if (outDir !== OUTPUT_DIR) return undefined;
 ...
 */
export async function captureFromToolCall(input: CaptureInput): Promise<void> {
  ...
  const cwd = typeof paramsObj?.cwd === "string" ? (paramsObj.cwd as string) : undefined;
  const workspaceDir = getWorkspaceDir(input.runId);

  slog(
    stateDir, T,
    `[capture] tool=${toolName} exec=${identity.executionId} member=${identity.memberId} role=${identity.role} cwd=${cwd ?? "-"} workspaceDir=${workspaceDir ?? "-"} startedAtMs=${startedAtMs} candidates=${candidates.length} ${JSON.stringify(candidates.slice(0, 20))}`,
  );
  if (candidates.length === 0) return;

  // 3. Resolve + verify each candidate on disk.
  const seen = new Set<string>();
  const enriched: EnrichedArtifact[] = [];
  ...
  for (const raw of candidates) {
    ...
    if (art) enriched.push(art);
    else rejected.push(abs);
  }
  slog(
    stateDir, T,
    `[capture] tool=${toolName} enriched=${enriched.length} rejected=${rejected.length} rejectedPaths=${JSON.stringify(rejected.slice(0, 20))}`,
  );
  ...
  const byGroup = new Map<string, { owner: Owner; artifacts: ManifestArtifact[] }>();
  ...
  await updateManifest(manifestPath, { teamId: T }, (manifest) => {
    for (const { owner, artifacts } of byGroup.values()) {
      ...
      for (const art of artifacts) upsertArtifact(group, art);
    }
  });

  slog(
    stateDir, T,
    `[capture] WROTE ${enriched.length} artifact(s) exec=${identity.executionId} owners=${JSON.stringify([...byGroup.values()].map((b) => `${b.owner.executionId}/${b.owner.memberId}:${b.artifacts.length}`))}`,
  );
  input.log?.(
    `[astron-agent-team] captured ${enriched.length} artifact(s) for team=${T} exec=${identity.executionId}`,
  );
}
