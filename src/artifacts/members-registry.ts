  ...
  await updateMembers(stateDir, meta.teamId, meta.leaderAgentId, (file) => {
    ...
    if (existing) {
      ...
      if (isReal && existing.worker.memberId.startsWith("unknown-")) {
        ...
        if (entry.label) existing.worker.label = entry.label;
        if (entry.runId) existing.worker.runId = entry.runId;
      }
      return;
    }
...
export function describeOutputOwner(
  ...
  if (specName) return { role: "worker", memberId, name: specName };
  ...
  if (cached) {
    for (const exec of cached.executions) {
      ...
      if (w) return { role: "worker", memberId, name: w.name };
    }
  }
  return undefined;
}
