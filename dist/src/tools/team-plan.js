...
const TeamPlanSchema = Type.Object({
    ...
    preferred_template_id: Type.Optional(Type.String({
        description: "Optional preferred template ID to bias selection.",
    })),
}, { additionalProperties: false });
export function createTeamPlanTool(stateDir, sessionKey) {
    return {
        name: "team_plan",
        description: "Analyze a task and suggest the best multi-agent team template. Returns ranked template suggestions and any reusable existing teams.",
        ...
        async execute(_toolCallId, params) {
            ...
            const reusableTeams = ranked.flatMap((r) => state.teams
                .filter((t) => t.templateId === r.template.id &&
                t.status === "ready" &&
            ...
            const guidance = suggestions.length > 0
                ? `Best match: "${suggestions[0].name}" (${suggestions[0].score}% keyword match). Use team_provision to create this team, or pick a different template.${reusableTeams.length > 0
