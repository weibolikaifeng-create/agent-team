...
const TeamCleanupSchema = Type.Object({ team_id: Type.String({ description: "ID of the team to clean up and remove." }) }, { additionalProperties: false });
export function createTeamCleanupToolCompat(stateDir, runtimeConfig, pruneAgentConfigFn) {
    return {
        ...
        async execute(_toolCallId, params) {
            ...
            if (!team) {
                return {
                    content: [{ type: "text", text: JSON.stringify({ error: `Team "${team_id}" not found.` }) }],
                };
            }
            const results = [];
            ...
            try {
                ...
                const { config: nextCfg, removedBindings, removedAllow } = pruneAgentConfigFn(cfg, team.leaderAgentId);
