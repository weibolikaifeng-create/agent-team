...
const TeamExecuteSchema = Type.Object({
    ...
    task_name: Type.Optional(Type.String({ description: "Short task name for display (max 15 chars). AI-generated summary of the task." })),
    steps: Type.Array(Type.String(), {
        description: 'Array of step descriptions for progress tracking. Include 1-2 preparation steps (e.g., "Set up agent team", "Break down tasks"), then one step per worker with their specific task. Example: ["Set up agent team", "Break down tasks and assign roles", "Search Specialist 1: Research DeepSeek V4 architecture", "Search Specialist 2: Research DeepSeek V4 training methods", "Report Writer: Write technical comparison report"]',
    }),
...
export function createTeamExecuteToolCompat(stateDir, sessionKey) {
    return {
        ...
        async execute(_toolCallId, params) {
            ...
            if (!team) {
                return {
                    content: [{ type: "text", text: JSON.stringify({ error: `Team "${team_id}" not found. Use team_provision first.` }) }],
                };
            }
            // Check team status
                ...
                content: [
                    {
                        ...
                        text: JSON.stringify({
                            ...
                            executionId,
                            execDir,
                            action_required: "CRITICAL: The team has NOT started yet. You MUST call sessions_send immediately with the parameters below to activate the Leader.\n\n" +
