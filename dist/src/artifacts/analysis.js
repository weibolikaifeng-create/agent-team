...
function readPluginLabelConfig(api) {
    return asObject(asObject(api?.pluginConfig).artifactLabel);
}
function resolveModelRef(api, cfg) {
    ...
    if (cfgProvider || cfgModel)
        return { provider: cfgProvider || undefined, model: cfgModel || undefined };
    ...
    const primary = typeof defaultsModel === "string" ? defaultsModel : asString(asObject(defaultsModel).primary);
    if (primary.includes("/")) {
...
async function runInferCli(prompt, modelRef, timeoutMs) {
...
function normalizeStatus(value) {
    ...
    if (s === "final" || s === "supporting" || s === "noise")
        return s;
    return undefined;
}
// ── prompt ───────────────────────────────────────────────────────────
 ...
 */
export async function labelArtifacts(input) {
    ...
    slog(stateDir, T, `[label] calling LLM (infer-cli) model=${formatModelRef(modelRef) || "-"} files=${files.length} timeoutMs=${timeoutMs}`);
    ...
    input.log?.(`[astron-agent-team] labeled ${labelByPath.size} artifact(s) for team=${T} exec=${identity.executionId} member=${identity.memberId}`);
}
